...
class TorchCodeMelSpectrogram(nn.Module):
    def __init__(
            self,
            n_mel_channels=80, sampling_rate=22050, filter_length=1024, hop_length=256, win_length=None,
    ...
    def forward(self, inp):
        ...
        if len(inp.shape) == 3:  # Automatically squeeze out the channels dimension if it is present (assuming mono-audio_tools)
            inp = inp.squeeze(1)
...
DEFAULT_MEL_NORM_FILE = "experiments/mel_norms.pth"


class TorchMelSpectrogram(nn.Module):
    def __init__(self, filter_length=1024, hop_length=256, win_length=1024, n_mel_channels=80, mel_fmin=0,
                 mel_fmax=8000, sampling_rate=22050, power=2, normalize=False, mel_norm_file=DEFAULT_MEL_NORM_FILE):
        super().__init__()
